...
async def list_agents(workspace: Path) -> list[AgentConfig]:
...
async def get_agent_logs(agent_name: str, n: int, workspace: Path) -> list:
...
async def get_execution_messages(execution_id: int, workspace: Path) -> list:
