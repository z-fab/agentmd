...
def test_keeps_only_latest_system_message():
    """Multiple SystemMessages from different runs — only the latest is kept."""
    messages = [
        SystemMessage(content="system prompt run 1"),
        HumanMessage(content="task 1"),
        AIMessage(content="done 1"),
        SystemMessage(content="system prompt run 2"),
        HumanMessage(content="task 2"),
        AIMessage(content="done 2"),
        SystemMessage(content="system prompt run 3"),
    ...
    assert system_msgs[0].content == "system prompt run 3"
    # All non-system messages should still be present
    non_system = [m for m in result if not isinstance(m, SystemMessage)]
