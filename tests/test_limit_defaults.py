...
def test_config_yaml_max_tool_calls():
    ...
    with patch("agent_md.config.models._get_global_limit_defaults", return_value={"max_tool_calls": 25}):
        s = SettingsConfig()
...
def test_config_yaml_max_execution_tokens():
    ...
    with patch("agent_md.config.models._get_global_limit_defaults", return_value={"max_execution_tokens": 200_000}):
        s = SettingsConfig()
...
def test_config_yaml_loop_detection():
    ...
    with patch("agent_md.config.models._get_global_limit_defaults", return_value={"loop_detection": False}):
        s = SettingsConfig()
...
def test_no_config_yaml_uses_hardcoded():
    ...
    with patch("agent_md.config.models._get_global_limit_defaults", return_value={}):
        s = SettingsConfig()
